...
#define unit64_t unsigned long long
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((unit64_t) (ptr) & (unit64_t) 3l);                     \
  (otri).tri = (triangle *)                                                   \
                  ((unit64_t) (ptr) ^ (unit64_t) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((unit64_t) (otri).tri | (unit64_t) (otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((unit64_t) (otri).tri[6] | (unit64_t) 2l)
...
#define infected(otri)                                                        \
  (((unit64_t) (otri).tri[6] & (unit64_t) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((unit64_t) (sptr) & (unit64_t) 1l);                  \
  (osub).ss = (subseg *)                                                      \
              ((unit64_t) (sptr) & ~ (unit64_t) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((unit64_t) (osub).ss | (unit64_t) (osub).ssorient)
...

{
  ...
  alignptr = (unit64_t) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (unit64_t) pool->alignbytes -
     (alignptr % (unit64_t) pool->alignbytes));
...

{
  ...
  alignptr = (unit64_t) (pool->pathblock + 1);
...

{
  ...
  alignptr = (unit64_t) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (unit64_t) m->triangles.alignbytes -
     (alignptr % (unit64_t) m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    alignptr = (unit64_t) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (unit64_t) m->subsegs.alignbytes -
       (alignptr % (unit64_t) m->subsegs.alignbytes));
...

{
  ...
  alignptr = (unit64_t) (getblock + 1);
  foundvertex = (char *) (alignptr + (unit64_t) m->vertices.alignbytes -
                          (alignptr % (unit64_t) m->vertices.alignbytes));
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (unit64_t) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (unit64_t) m->triangles.alignbytes -
                         (alignptr %
                          (unit64_t) m->triangles.alignbytes));
