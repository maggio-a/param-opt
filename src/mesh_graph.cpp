...
void DoRemesh(Mesh& shell)
{
    ...
    std::cout << startFN2 << " faces are not hole filling after the remesh " << max_i << std::endl;

    bool die = false;
    for (auto& sf : shell.face) {
        if (sf.holeFilling == false && ia[sf] == -1) {
            //assert(ia[sf] != -1);
            std::cout << "face " << tri::Index<Mesh>(shell, sf) << " has lost its index attribute " << std::endl;
            die = true;
        }
    }
    assert(!die);
...
void CloseShellHoles(Mesh& shell)
{
    ...
    for (std::size_t i = 0; i < info.vBoundaryFaces.size(); ++i) {
        ...
        while (fi != shell.face.end()) {
            /* the triangulator wraps faces clock-wise, so swap two vertices */
...
/*
    ...
    if (info.vBorderFaces.size() > 1) {
        ...
        // close everything that is selected
        //tri::Hole<Mesh>::EarCuttingFill<tri::MinimumWeightEar<Mesh>>(shell, shell.FN(), true);
        tri::io::Exporter<Mesh>::Save(shell, "close_hole_before.obj", tri::io::Mask::IOM_FACECOLOR | tri::io::Mask::IOM_VERTTEXCOORD);
        tri::Hole<Mesh>::EarCuttingIntersectionFill<tri::SelfIntersectionEar<Mesh>>(shell, shell.FN(), true);
        tri::io::Exporter<Mesh>::Save(shell, "close_hole_after.obj", tri::io::Mask::IOM_FACECOLOR | tri::io::Mask::IOM_VERTTEXCOORD);
    }
